...
public class Teki2 extends AbstractMono {
    private static final int[] ids = {R.drawable.book3};
    private int dpindex;
    private Vect[] dps = {new Vect(rand(2,true), 2), new Vect(rand(2,true), 2)};
    private double dpcycle = 500;
    private double dpcounter;
    public Teki2(Context context) {
        super(context, ids);
    }
    public Teki2(Context context, int x, int y) {
        super(context, ids);
        set(x, y);
        dp.set(dps[0]);
        dpindex = 0;
